...
contract MockPoolManager {
    ...
    event Swap(bytes32 indexed poolId, address indexed sender, bool zeroForOne, uint256 amountIn, uint256 amountOut);

    event PoolInitialized(
        bytes32 indexed poolId, address currency0, address currency1, uint256 reserve0Amount, uint256 reserve1Amount
    );
    ...
    function initializePool(PoolKey calldata key, uint256 amount0, uint256 amount1) external {
        ...
        emit PoolInitialized(poolId, Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), amount0, amount1);
    }
    ...
    function getReserves(bytes32 poolId) external view returns (uint256 _reserve0, uint256 _reserve1) {
